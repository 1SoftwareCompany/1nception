...
public class InMemoryAggregateRootAtomicAction : IAggregateRootAtomicAction
{
    ...
    public async Task<One.Inception.Userfull.Result<bool>> ExecuteAsync(AggregateRootId aggregateRootId, int aggregateRootRevision, Func<Task> action)
    {
        var result = new One.Inception.Userfull.Result<bool>(false);
                ...
                try
                {
                    ...
                    if (revision.CompareAndSet(aggregateRootRevision - 1, aggregateRootRevision))
                    {
                        try
                        {
                            ...
                            return One.Inception.Userfull.Result.Success;
                        }
                        catch (Exception)
                        {
        ...
        {
            return One.Inception.Userfull.Result.Error(ex);
        }
    }
