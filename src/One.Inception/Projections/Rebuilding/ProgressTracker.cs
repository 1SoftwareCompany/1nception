...
public class ProgressTracker
{
    ...
    public ProgressTracker(IMessageCounter messageCounter, IInceptionContextAccessor contextAccessor, IPublisher<ISystemSignal> signalPublisher, ProjectionVersionHelper projectionVersionHelper, ILogger<ProgressTracker> logger)
    {
        EventTypeProcessed = new Dictionary<string, StrongBox<ulong>>();
        tenant = contextAccessor.Context.Tenant;
                    ...
                    notifier = Task.Run(async () =>
                    {
                        try
                        {
                            while (true)
                            {
                                ...
                                await signalPublisher.PublishAsync(progressSignalche).ConfigureAwait(false);
