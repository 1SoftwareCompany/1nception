...
public class ReplayNotPersistedProjection_Job_JobFactory : IProjection_JobFactory
{
    private readonly ReplayNotPersistedProjection_Job job;
    private readonly IInceptionContextAccessor contextAccessor;
    private readonly BoundedContext boundedContext;

    public ReplayNotPersistedProjection_Job_JobFactory(ReplayNotPersistedProjection_Job job, IOptions<BoundedContext> boundedContext, IInceptionContextAccessor contextAccessor)
    {
        this.job = job;
    ...
    public IInceptionJob<object> CreateJob(ProjectionVersion version, ReplayEventsOptions replayEventsOptions, VersionRequestTimebox timebox)
    {
        ...
        job.BuildInitialData(() => new ReplayNotPersistedProjection_JobData()
        {
