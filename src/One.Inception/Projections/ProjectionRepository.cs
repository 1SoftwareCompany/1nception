...
{
    private static readonly ILogger log = InceptionLogger.CreateLogger(typeof(ProjectionRepository));
    private static readonly Action<ILogger, Exception> LogProjectionInstanceNotFound = LoggerMessage.Define(LogLevel.Debug, InceptionLogEvent.ProjectionRead, "Projection instance not found.");
    private static readonly Action<ILogger, Exception> LogProjectionLoadError = LoggerMessage.Define(LogLevel.Error, InceptionLogEvent.ProjectionRead, "Unable to load projection.");
    private static readonly Action<ILogger, Exception> LogProjectionLiveVersionMissing = LoggerMessage.Define(LogLevel.Error, InceptionLogEvent.ProjectionRead, "Unable to find projection `live` version.");
    private static readonly Action<ILogger, Exception> LogProjectionWriteError = LoggerMessage.Define(LogLevel.Error, InceptionLogEvent.ProjectionWrite, "Failed to write in projection.");
    ...
    public ProjectionRepository(IInceptionContextAccessor contextAccessor, IProjectionStore projectionStore, IHandlerFactory handlerFactory, ProjectionHasher projectionHasher)
    {
    ...
    public async Task SaveAsync(Type projectionType, IEvent @event)
    {
        ...
        if (projection is not null)
        {
            ...
            catch (Exception ex)
            {
                ...
                log.LogWarning(ex, "Failed to save event {inception.Messageevent} in projection {projection}.", @event.GetType().Name, projectionType.Name);
    ...
    private async Task<ReadResult<ProjectionVersionsHandler>> GetProjectionVersionsFromStoreAsync(string projectionName)
    {
        try
        {
            ...
            ProjectionVersionManagerId versionId = new ProjectionVersionManagerId(projectionName, contextAccessor.Context.Tenant);
