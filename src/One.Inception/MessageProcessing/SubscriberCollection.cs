...
/// <summary>
/// The responsibility of this collection is to collect and work with all the message subscribers in the inception infrastructure.
...
public sealed class SubscriberCollection<T> : ISubscriberCollection<T>
{
    ...
    /// <summary>
    /// Adds a new subscriber to the inception infrastructure with intent to notify all interested parties when a new subscriber comes in (e.g. for creating queues etc.)
    /// </summary>
    ...
    public IEnumerable<ISubscriber> GetInterestedSubscribers(InceptionMessage message)
    {
        try
        {
