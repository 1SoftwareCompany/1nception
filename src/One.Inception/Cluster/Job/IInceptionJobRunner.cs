...
/// <summary>
/// Allows executing a <see cref="IInceptionJob{TData}"/> in a cluster environment.
/// </summary>
/// <remarks>Consider implementing <see cref="IClusterOperations"/> as well.</remarks>
public interface IInceptionJobRunner : IDisposable
{
    /// <summary>
    /// Executes the <see cref="IInceptionJob{TData}"/>
    /// </summary>
    /// <param name="job">The job.</param>
    /// <returns>Returns the <see cref="JobExecutionStatus"/> of the job after execution.</returns>
    Task<JobExecutionStatus> ExecuteAsync(IInceptionJob<object> job, CancellationToken cancellationToken = default);

    JobManager JobManager { get; }
}

public abstract class AbstractInceptionJobRunner : IInceptionJobRunner
{
    private readonly JobManager jobManager;

    public AbstractInceptionJobRunner(JobManager jobManager)
    {
    ...
    protected abstract Task<JobExecutionStatus> ExecuteInternalAsync(IInceptionJob<object> job, CancellationToken cancellationToken = default);
