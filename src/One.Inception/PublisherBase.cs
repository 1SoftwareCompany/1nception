...
internal class HeadersPublishHandler : DelegatingPublishHandler
{
    private readonly ITenantResolver<IMessage> tenantResolver;
    private readonly BoundedContext boundedContext;

    public HeadersPublishHandler(ITenantResolver<IMessage> tenantResolver, IOptions<BoundedContext> boundedContextOptions)
    {
...
internal class LoggingPublishHandler : DelegatingPublishHandler
{
    ...
    private static readonly Action<ILogger, string, Exception> LogPublishOK = LoggerMessage.Define<string>(LogLevel.Information, InceptionLogEvent.PublishOk, "Publish {inception_MessageType} - OK");
    private static readonly Action<ILogger, string, Exception> LogPublishError = LoggerMessage.Define<string>(LogLevel.Error, InceptionLogEvent.PublishError, "Publish {inception_MessageType} - Fail");
    ...
    protected internal override PublishResult PublishInternal(InceptionMessage message)
    {
        using (logger.BeginScope(s => s.AddScope(Log.MessageId, message.Id.ToString())))
        {
            ...
            catch (Exception ex) when (True(() => logger.LogError(ex, "Failed to publish message {inception_MessageType}. {@inception_Message}", message.GetMessageType().Name, message)))
            {
...
internal class ActivityPublishHandler : DelegatingPublishHandler
{
    ...
    private Activity StartActivity(InceptionMessage message)
    {
...
public abstract class PublisherBase<TMessage> : PublisherHandler, IPublisher<TMessage> where TMessage : IMessage
{
    ...
    public virtual bool Publish(byte[] messageRaw, Type messageType, string tenant, Dictionary<string, string> messageHeaders)
    {
        ...
        InceptionMessage inceptionMessage = new InceptionMessage(messageRaw, messageType, messageHeaders);
